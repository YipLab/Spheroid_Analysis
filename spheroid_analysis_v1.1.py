...
threshold_edge = 25  # TODO: 8-bit grey value for edge thresholding to remove background

min_area = 40  # Max area for a singular spheroid
max_area = 140000  # Max area for a singular spheroid

shift_distance = 50  # TODO: calculate maximum shift_distance of ROI with voxel

ds_factor = [round(max(voxel) / voxel[0]), round(max(voxel) / voxel[1]),
             round(max(voxel) / voxel[2])]  # factor for downscaling
...
def padding(obj_list):  # apply padding to and conform ROIs to full-res, save location and resolution
    ...
    for obj in range(len(obj_list)):
        ...
        for roi in range(len(obj_list[obj])):
            ...
            padded = np.array([int((obj_list[obj][roi][0] - 1) * ds_factor[0] - pad[0]),
                               int((obj_list[obj][roi][1] - 1) * ds_factor[1] - pad[1]),
                               int((obj_list[obj][roi][2] - 1) * ds_factor[2] - pad[2]),
                               int((obj_list[obj][roi][3] + 1) * ds_factor[0] + pad[0]),
                               int((obj_list[obj][roi][4] + 1) * ds_factor[1] + pad[1]),
                               int((obj_list[obj][roi][5] + 1) * ds_factor[2] + pad[2])])
...
def save_rois_4d(pad, res5, start, num, maxi, files, filename):
    ...
    im_out = np.empty((maxi[0], maxi[1], maxi[2] * num), dtype=np.uint16)  # empty hyperstack

    for vol in range(num):  # the number of frames the ROI spans

        for i, file in enumerate(
                files[(start + vol) * z_depth:len(files) - extra]):  # search files excluding already saved files

            if (i >= pad[vol][2] and z < pad[vol][5] - pad[vol][2]):  # use provided cropping window

                im_r = readImg(file)  # read image files with temp variable im_r

                # Take the avalible portion of the image in the event of cut out
                im_out[0:res5[vol][0], 0:res5[vol][1], vol * maxi[2] + z] = im_r[pad[vol][0]:pad[vol][0] + res5[vol][0],
                                                                            pad[vol][1]:pad[vol][1] + res5[vol][1]]
                z += 1
        z = 0

        # i = (start+vol)*z_depth
        # for j , file in enumerate(files[i : i+maxi[2]]): # search files excluding already saved files
        #     im_r = tf.imread(files_path + "/" + file) # read image files with temp variable im_r
        #     # Take the avalible portion of the image in the event of cut out
        #     im_out[0:res5[vol][0],0:res5[vol][1],vol*maxi[2] + j] = im_r[pad[vol][0]:pad[vol][0]+res5[vol][0], pad[vol][1]:pad[vol][1]+res5[vol][1]]
...
def save_rois(roi, files, count,
              exception):  # unused 2d saving function, input == ('roi object'), img_in == ('2d arrays')
    ...
    im_o = np.empty(res2, dtype=np.uint16)  # copy dimensions
    ...
    if (exception == False):
        ...
        tf.imsave(save_path + "rois/roi_" + str(count) + "_exception" + ".tif",
                  np.moveaxis(im_o.astype(np.uint16), -1, 0))  # exception raised in filename

    return


def save_rois_3d(roi, count, i, j,
                 exception):  # unused 2d saving function, input == ('roi object'), img_in == ('3d stacks')
    ...
    im_o = np.empty(res2)  # copy dimensions

    for file in exp_files:
        if (file[-13:] == "exception.tif"):
            if (file[-15] == str(i)):
                im_read = np.moveaxis(tf.imread(save_path + "/rois/" + file), 0,
                                      -1)  # read image files with temp variable im_r
                im_o = im_read[dim[0]:dim[0] + res2[0], dim[1]:dim[1] + res2[1],
                       dim[2]:dim[2] + res2[2]]  # save cropped area

    tf.imsave(save_path + "rois/roi_" + str(count) + "_reprocessed_" + str(i) + '-' + str(j) + ".tif",
              np.moveaxis(im_o.astype(np.uint16), -1, 0))  # exception raised in filename
...
def save_hyperstack(ndarray, method, filename, path):  # convert a 4d array to a 3d hyperstack image
    ...
    im_out = np.empty([res3[0], res3[1], int(res3[2] * res3[3])])  # empty image
...
def draw_rois(bbox, greyscale, k):  # draw a bounding box on a 8 bit 4d array (configured as im_8) with greyscale value
    ...
    xx, yy = draw.rectangle_perimeter(begin, end=end, shape=im_8[:, :, :, k].shape, clip=True)
...
def exception_advanced_threshold(ndarray, pad, dim, big_bbox, k):
    ...
    im_label = measure.label(threshold_edge < im_adv, connectivity=1)
    rois_thr = measure.regionprops(im_label)
    ...
    for j, roi in enumerate(rois_thr):
        ...
        if (roi.area <= 70 or (roi.bbox[0] == 0 or roi.bbox[1] == 0 or roi.bbox[2] == 0
                               or roi.bbox[3] == im_d.shape[0] or roi.bbox[4] == im_d.shape[1] or roi.bbox[5] ==
                               im_d.shape[2])):
            continue
        else:
...
def get_distance(bbox1, bbox2):  # return centroidal distance between two bounding boxes
    ...
    return np.sqrt((bbox2_c[0] - bbox1_c[0]) ** 2 + (bbox2_c[1] - bbox1_c[1]) ** 2 + (bbox2_c[2] - bbox1_c[2]) ** 2)
...
im = np.empty([im.shape[0], im.shape[1], z_depth], dtype=np.uint16)  # make empty image
res = [math.ceil(im.shape[0] / ds_factor[0]), math.ceil(im.shape[1] / ds_factor[1]),
       math.ceil(im.shape[2] / ds_factor[2])]

im_d = np.empty([res[0], res[1], res[2], frame], dtype=np.uint16)  # creating a 4d array

print('\nOriginal Volume has size of ' + str(im.shape[0]) + ' by ' + str(im.shape[1]) + ' by ' + str(
    im.shape[2]) + ' with', frame, 'frames')
...
for i , file in enumerate(tqdm(files[0:len(files)-extra])): # enumerate files and iterate progress bar
    if (i%z_depth == 0 and i != 0):
        j += 1

    im_r = readImg(file) # read image files with temp variable im_r
    im_d[:,:,(i%z_depth),j] = im_r[::ds_factor[0],::ds_factor[1]] # Volume_Downsample and apply onto array

# elapsed = time.time() -start
print('\nDownsampled Volume has size of ' + str(im_d.shape[0]) + ' by ' + str(im_d.shape[1]) + ' by ' + str(im_d.shape[2]))
...
im_g = np.empty_like(im_d, dtype = np.uint16)
...
for k in range (frame):
    im_l = measure.label(im_t[:,:,:,k], connectivity=1)
    rois = measure.regionprops(im_l)
...
# ____________________________________________________
# Save/Load bbox on/from a npy file

# np.save(save_path + 'rois_bbox.npy')
# print('\nbbox saved.')

# rois_bbox = np.load(save_path + 'rois_bbox.npy')
# print('\nbbox loaded.')
...
min_distance = np.sqrt(res[0] ** 2 + res[1] ** 2)  # Preset maximum distance (diagonal) across the downscaled image
min_roi = 0
...
for vol in range(frame):  # Scan each frame for ROIs that does not span the whole z_depth (starts from a higher index)
    for r in range(len(rois_bbox[vol])):
        ...
        for f in range(frame - vol - 1):  # Iterate  throught the frames
            ...
            r_empty = 0  # counter for conting how many empty bbox has been used
            ...
            for r_next in range(len(rois_bbox[f + 1])):  # The number of ROIs on the next frame of frame == f
                ...
                if (min_d >= get_distance(rois_link[f], rois_bbox[f + 1][
                    r_next])):  # ROIs on the next frame that is closest to rois_bbox[f][r] (rois_link[f])
            ...
            if (shift_distance >= get_distance(rois_link[f], rois_bbox[f + 1][min_roi])
                    and (get_volume(rois_link[f]) <= 4 * get_volume(rois_bbox[f + 1][min_roi]) or get_volume(
                        rois_link[f]) <= 0.25 * get_volume(rois_bbox[f + 1][min_roi]))):  # general_case
                ...
                rois_link.append(
                    rois_link[f])  # if can't find matching bbox in the next frame, look heads up to 3 frames
                r_empty += 1

            elif (r_empty == 3):  # if nothing is found in 3 frames
                del (rois_link[f])
                del (rois_link[f - 1])
                del (rois_link[f - 2])  # delete previous empty instances
...
for obj in range(len(rois_list)):
    res4 = np.empty(3, dtype=np.int32)  # empty 3d numpy array

    # max_box = 0 # set max volume for saving

    for roi in range(len(rois_list[obj])):
        centroid_list.append(get_centroid(rois_list[obj][roi]))  # append centroids to a list
        maxi_res.append(get_res(rois_list[obj][roi]))
    #     if get_volume(rois_list[obj][roi])> max_box:
    #         max_box = get_volume(rois_list[obj][roi]) # find max volume and pass it
    #         res4 = get_res(rois_list[obj][roi])

    res4 = max_res(maxi_res)

    res_list.append(get_conformed_bbox(centroid_list, res4))  # conform bboxes with centroids and new dimensions
...
for obj in tqdm(range(len(pad_list))):  # save timelapse, text data and iterate progress bar

    outfile = open(str(rois_path + "roi_" + str(obj) + ".txt"), 'w')  # save to txt file

    outfile.write('Crop dimension:\n' + str(full_res_list[obj][0]) + '\n')
    outfile.write('Starting frame:\n' + str(start_frame[obj]) + '\n')
    outfile.write('Frame(s) per lapse:\n' + str(len(rois_list[obj])) + '\nDownsampled ROI bounding boxes:\n')

    outfile.write('\n'.join(str(item) for item in rois_list[obj]))  # save ROI dimension fpr reference
    outfile = 0

    max_r = max_res(full_res_list[obj])  # obtain maxium dimension

    # import and save hyper stacks with designated namings
    obj_4d = save_rois_4d(pad_list[obj], full_res_list[obj], start_frame[obj], len(rois_list[obj]), max_r, files,
                          str("roi_" + str(obj) + ".tif"))  # croped 4d numpy array for each object

elapsed = time.time() - start
print('\nROI Saving time: ' + str(elapsed / 60) + ' minutes')
print('Done.')
